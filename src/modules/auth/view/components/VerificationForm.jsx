...
const VerificationForm = () => {
  ...
  const handleBrowseApp = useCallback(() => navigate("/student-home"), [navigate]);
  const handleReturnHome = useCallback(() => navigate("/"), [navigate]);

  return (
    <div className="min-h-screen bg-[#F7F7F5] flex flex-col relative">
      <header className="shrink-0 px-5 sm:px-8 h-16 flex items-center justify-between border-b border-black/7 bg-white/90 backdrop-blur-md z-20 fixed w-full">
        <div onClick={handleReturnHome} className="flex items-center gap-2 cursor-pointer">
          <div className="w-8 h-8 rounded-xl bg-[#F07B3A] flex items-center justify-center">
            <svg className="w-5 h-5 text-white" viewBox="0 0 24 24" fill="currentColor">
              <path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z" />
            </svg>
          </div>
          <span style={{ fontFamily: "Syne", fontWeight: 800 }} className="text-[#111] text-lg">
            Sharezi
          </span>
        </div>
        ...
        <div className="w-full max-w-[420px]">
          <div className="flex justify-center mb-7 u1">
            <div className="clock-wrap w-20 h-20 rounded-full bg-[#FEF9EC] border-2 border-[#F5B942]/30 flex items-center justify-center">
              <span className="icon" style={{ fontSize: "38px", color: "#F5B942" }}>schedule</span>
            </div>
          </div>
          ...
          <div className="bg-white border border-black/7 rounded-2xl p-5 mb-4 u3">
            ...
            {documents.map((doc, index) => (
              <div key={index} className="check-item">
                ...
                <span className="text-sm text-[#111]/70">{doc}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-3 u5">
            ...
            <button onClick={handleReturnHome} className="btn-outline">
              Return to Home
            </button>
          </div>

          <footer className="text-center text-sm text-[#111]/35 mt-6">
            Need help?{" "}
            <a href="#" className="text-[#3DBDA8] font-semibold hover:underline">
              Contact Support
            </a>
          </footer>
        </div>
